<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Admin Dashboard CMS | UMKM Desa Kumpeh</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
    <script src="https://unpkg.com/lucide@latest"></script>
</head>
<body class="bg-stone-100 text-stone-900 min-h-screen flex font-sans">

    <!-- Sidebar -->
    <aside class="w-64 bg-[#1E3822] text-stone-200 flex flex-col justify-between shrink-0">
        <div class="p-6 space-y-6">
            <div class="flex items-center space-x-3">
                <div class="w-9 h-9 rounded-xl bg-[#C85A32] text-white flex items-center justify-center font-bold text-base shadow">
                    GK
                </div>
                <div>
                    <h2 class="font-bold text-sm text-white leading-tight">Admin CMS Kumpeh</h2>
                    <span class="text-[10px] text-amber-300">Laravel 12 Engine</span>
                </div>
            </div>

            <nav class="space-y-1 pt-4 text-xs font-bold">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 px-3.5 py-3 rounded-xl bg-[#2E4F32] text-white">
                    <i data-lucide="layout-dashboard" class="w-4 h-4 text-amber-400"></i>
                    <span>Dashboard Utama</span>
                </a>
                <a href="{{ route('admin.settings.index') }}" class="flex items-center space-x-3 px-3.5 py-3 rounded-xl hover:bg-[#2E4F32] text-stone-300 hover:text-white transition-colors">
                    <i data-lucide="sliders" class="w-4 h-4 text-amber-300"></i>
                    <span>Pengaturan Website (CMS)</span>
                </a>
                <a href="{{ route('admin.products.index') }}" class="flex items-center space-x-3 px-3.5 py-3 rounded-xl hover:bg-[#2E4F32] text-stone-300 hover:text-white transition-colors">
                    <i data-lucide="shopping-bag" class="w-4 h-4 text-emerald-400"></i>
                    <span>Kelola Produk Olahan</span>
                </a>
                <a href="{{ route('admin.articles.index') }}" class="flex items-center space-x-3 px-3.5 py-3 rounded-xl hover:bg-[#2E4F32] text-stone-300 hover:text-white transition-colors">
                    <i data-lucide="newspaper" class="w-4 h-4 text-blue-400"></i>
                    <span>Kelola Artikel Berita</span>
                </a>
                <a href="{{ route('home') }}" target="_blank" class="flex items-center space-x-3 px-3.5 py-3 rounded-xl hover:bg-[#2E4F32] text-stone-300 hover:text-white transition-colors">
                    <i data-lucide="globe" class="w-4 h-4 text-amber-300"></i>
                    <span>Lihat Website Publik</span>
                </a>
            </nav>
        </div>

        <div class="p-6 border-t border-white/10">
            <form action="{{ route('admin.logout') }}" method="POST">
                @csrf
                <button type="submit" class="w-full flex items-center justify-center space-x-2 bg-red-600/20 hover:bg-red-600 text-red-300 hover:text-white py-2.5 rounded-xl text-xs font-bold transition-colors">
                    <i data-lucide="log-out" class="w-4 h-4"></i>
                    <span>Keluar (Logout)</span>
                </button>
            </form>
        </div>
    </aside>

    <!-- Main Content -->
    <main class="flex-grow p-8 space-y-8 overflow-y-auto">
        
        <!-- Header -->
        <div class="flex items-center justify-between pb-4 border-b border-stone-200">
            <div>
                <h1 class="font-bold text-2xl text-stone-900">Dashboard Pengelolaan CMS</h1>
                <p class="text-xs text-stone-500 mt-1">Selamat datang kembali, Administrator UMKM Desa Kumpeh</p>
            </div>
            <a href="{{ route('admin.products.create') }}" class="bg-[#C85A32] hover:bg-[#A44322] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow flex items-center space-x-2">
                <i data-lucide="plus" class="w-4 h-4"></i>
                <span>Tambah Produk Baru</span>
            </a>
        </div>

        <!-- Summary Stat Cards -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm flex items-center justify-between">
                <div>
                    <div class="text-xs text-stone-500 font-bold uppercase tracking-wider">Total Produk</div>
                    <div class="text-3xl font-bold text-stone-900 mt-1">{{ $totalProducts }}</div>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-700 flex items-center justify-center">
                    <i data-lucide="shopping-bag" class="w-6 h-6"></i>
                </div>
            </div>

            <div class="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm flex items-center justify-between">
                <div>
                    <div class="text-xs text-stone-500 font-bold uppercase tracking-wider">Total Artikel Berita</div>
                    <div class="text-3xl font-bold text-stone-900 mt-1">{{ $totalArticles }}</div>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-blue-100 text-blue-700 flex items-center justify-center">
                    <i data-lucide="newspaper" class="w-6 h-6"></i>
                </div>
            </div>

            <div class="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm flex items-center justify-between">
                <div>
                    <div class="text-xs text-stone-500 font-bold uppercase tracking-wider">Cerita Warga</div>
                    <div class="text-3xl font-bold text-stone-900 mt-1">{{ $totalStories }}</div>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-amber-100 text-amber-700 flex items-center justify-center">
                    <i data-lucide="users" class="w-6 h-6"></i>
                </div>
            </div>
        </div>

        <!-- Recent Products Table -->
        <div class="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden space-y-4 p-6">
            <div class="flex items-center justify-between">
                <h3 class="font-bold text-base text-stone-900">Daftar Produk Terbanyak</h3>
                <a href="{{ route('admin.products.index') }}" class="text-xs font-bold text-[#C85A32] hover:underline">Lihat Semua</a>
            </div>

            <table class="w-full text-left text-xs">
                <thead class="bg-stone-50 text-stone-600 uppercase font-bold border-b border-stone-200">
                    <tr>
                        <th class="p-3">Nama Produk</th>
                        <th class="p-3">Harga</th>
                        <th class="p-3">Stok</th>
                        <th class="p-3">Kemasan</th>
                        <th class="p-3 text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-stone-100">
                    @foreach($recentProducts as $p)
                    <tr>
                        <td class="p-3 font-bold text-stone-900">{{ $p->name }}</td>
                        <td class="p-3 font-semibold text-[#2E4F32]">Rp {{ number_format($p->price, 0, ',', '.') }}</td>
                        <td class="p-3 font-bold">{{ $p->stock }} pcs</td>
                        <td class="p-3 text-stone-500">{{ $p->packaging ?? '-' }}</td>
                        <td class="p-3 text-right space-x-2">
                            <a href="{{ route('admin.products.edit', $p->id) }}" class="text-blue-600 font-bold hover:underline">Edit</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        </div>

    </main>

    <script>
        lucide.createIcons();
    </script>
</body>
</html>
