...
<head>
    ...
    <title>Kelola Cerita Warga | Admin CMS Kumpeh</title>
...
<body class="bg-stone-100 text-stone-900 min-h-screen flex font-sans">
    ...
    <main class="flex-grow p-8 space-y-6 overflow-y-auto">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-stone-200">
            <div>
                <h1 class="font-bold text-2xl text-stone-900">Kelola Cerita & Tokoh Warga</h1>
                <p class="text-xs text-stone-500 mt-1">Tambah, edit, dan hapus testimoni serta kisah inspiratif warga Desa Kumpeh</p>
            </div>
            <a href="{{ route('admin.stories.create') }}" class="bg-[#C85A32] hover:bg-[#A44322] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow flex items-center space-x-2">
                <i data-lucide="plus" class="w-4 h-4"></i>
                <span>Tambah Cerita Warga Baru</span>
            </a>
        </div>
            ...
            <table class="w-full text-left text-xs">
                <thead class="bg-stone-50 text-stone-600 uppercase font-bold border-b border-stone-200">
                    <tr>
                        ...
                        <th class="p-3">Nama Tokoh</th>
                        <th class="p-3">Peran / Jabatan</th>
                        <th class="p-3">Kutipan Utama (Quote)</th>
                ...
                <tbody class="divide-y divide-stone-100">
                    ...
                    <tr>
                        <td class="p-3">
                            <img src="{{ $s->photo_url }}" alt="{{ $s->name }}" class="w-12 h-12 rounded-xl object-cover border border-stone-200">
                        </td>
                        <td class="p-3 font-bold text-stone-900">{{ $s->name }}</td>
                        <td class="p-3 font-semibold text-[#2E4F32]">{{ $s->role }}</td>
                        <td class="p-3 text-stone-600 max-w-xs truncate">"{{ $s->quote }}"</td>
                        <td class="p-3 text-right space-x-3">
                            <a href="{{ route('admin.stories.edit', $s->id) }}" class="text-blue-600 font-bold hover:underline">Edit</a>
                            <form action="{{ route('admin.stories.destroy', $s->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus cerita ini?')">
                                @csrf
                                @method('DELETE')
