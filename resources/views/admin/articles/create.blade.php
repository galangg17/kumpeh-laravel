...
<head>
    ...
    <title>Tambah Artikel Berita Baru | Admin CMS Kumpeh</title>
...
<body class="bg-stone-100 text-stone-900 min-h-screen flex font-sans">
    ...
    <main class="flex-grow p-8 space-y-6 overflow-y-auto">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-stone-200">
            <div>
                <h1 class="font-bold text-2xl text-stone-900">Tambah Artikel Berita Baru</h1>
                <p class="text-xs text-stone-500 mt-1">Tulis artikel berita & upload foto utama rilis UMKM Desa Kumpeh</p>
            </div>
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm space-y-6 max-w-3xl">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-stone-700 mb-1">Judul Artikel Berita</label>
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Judul artikel berita..." class="w-full border border-stone-300 rounded-xl px-4 py-2.5 text-xs font-bold focus:outline-none focus:border-[#2E4F32]">
                </div>

                <div>
                    <label class="block text-xs font-bold text-stone-700 mb-1">Penulis / Sumber</label>
                    <input type="text" name="author" value="{{ old('author', 'Tim UMKM Kumpeh') }}" required class="w-full border border-stone-300 rounded-xl px-4 py-2.5 text-xs font-bold focus:outline-none focus:border-[#2E4F32]">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-stone-700 mb-1">Ringkasan Artikel (Excerpt)</label>
                <input type="text" name="excerpt" value="{{ old('excerpt') }}" required placeholder="Ringkasan singkat 1-2 kalimat..." class="w-full border border-stone-300 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-[#2E4F32]">
            </div>

            <div>
                <label class="block text-xs font-bold text-stone-700 mb-1">Isi Berita Lengkap</label>
                <textarea name="content" rows="6" required placeholder="Tuliskan isi artikel berita..." class="w-full border border-stone-300 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-[#2E4F32]">{{ old('content') }}</textarea>
            </div>

            <!-- Upload File or URL Section -->
            <div class="bg-stone-50 p-4 rounded-xl border border-stone-200 space-y-3">
                <label class="block text-xs font-bold text-stone-800">Foto Berita Utama:</label>
                ...
                <div>
                    <label class="block text-[11px] font-bold text-stone-600 mb-1">Opsi B: Masukkan URL Gambar (Direct Link)</label>
                    <input type="url" name="featured_image" value="{{ old('featured_image') }}" placeholder="https://..." class="w-full border border-stone-300 rounded-xl px-4 py-2 text-xs focus:outline-none focus:border-[#2E4F32]">
                </div>
            </div>

            <div class="flex items-center space-x-2 pt-2">
                <input type="checkbox" name="is_published" value="1" id="is_published" checked class="rounded text-[#C85A32]">
                <label for="is_published" class="text-xs font-bold text-stone-800">Publikasikan Langsung ke Website</label>
            </div>
