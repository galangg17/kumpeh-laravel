...
<head>
    ...
    <title>Kelola Berita & Artikel | Admin CMS Kumpeh</title>
...
<body class="bg-stone-100 text-stone-900 min-h-screen flex font-sans">
    ...
    <aside class="w-64 bg-[#1E3822] text-stone-200 flex flex-col justify-between shrink-0">
        <div class="p-6 space-y-6">
            ...
            <nav class="space-y-1 pt-4 text-xs font-bold">
                ...
                <a href="{{ route('admin.stories.index') }}" class="flex items-center space-x-3 px-3.5 py-3 rounded-xl hover:bg-[#2E4F32] text-stone-300 hover:text-white transition-colors">
    ...
    <main class="flex-grow p-8 space-y-6 overflow-y-auto">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-stone-200">
            <div>
                <h1 class="font-bold text-2xl text-stone-900">Kelola Artikel Berita & Jurnal</h1>
                <p class="text-xs text-stone-500 mt-1">Tambah, edit, dan hapus rilis berita pengembangan UMKM Desa</p>
            </div>
            <a href="{{ route('admin.articles.create') }}" class="bg-[#C85A32] hover:bg-[#A44322] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow flex items-center space-x-2">
                <i data-lucide="plus" class="w-4 h-4"></i>
                <span>Tambah Artikel Berita Baru</span>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden p-6">
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-stone-100">
                    ...
                    <tr>
                        <td class="p-3">
                            <img src="{{ $a->featured_image }}" alt="{{ $a->title }}" class="w-12 h-12 rounded-xl object-cover border border-stone-200">
                        </td>
                        <td class="p-3 font-bold text-stone-900 max-w-xs truncate">{{ $a->title }}</td>
                        <td class="p-3 text-stone-600">{{ $a->author }}</td>
                        <td class="p-3">
                        ...
                        <td class="p-3 text-right space-x-3">
                            <a href="{{ route('admin.articles.edit', $a->id) }}" class="text-blue-600 font-bold hover:underline">Edit</a>
                            <form action="{{ route('admin.articles.destroy', $a->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini?')">
                                @csrf
                                @method('DELETE')
